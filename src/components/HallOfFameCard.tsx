...
export function HallOfFameCard({ entry }: { entry: HallOfFameEntry }) {
  return (
    <article className="rounded-lg border border-line bg-surface p-5">
      ...
      <h3 className="mt-3 text-base font-semibold tracking-tight">{entry.teamName}</h3>
      <p className="mt-1 font-mono text-base text-fg">{entry.placement}</p>

      <dl className="mt-4 space-y-1 text-base text-fg-muted">
        <div>
